...
public class TestGrammar extends TyphonTest {
    ...
    public static Collection<Object[]> data() {
		return TyphonTest.makeData(
			...
			new CaseValid("import \"\";"),
			new CaseValid("import \"\" as y;"),
			new CaseValid("@a @b import \"\";"),
			...
			//comboType
			new CaseValid("(int && float)", (parser)->{parser.type();}),
			new CaseValid("(int && float && Any && List<int>)", (parser)->{parser.type();}),
			new CaseValid("(int && (Integer && Real) && Any)", (parser)->{parser.type();}),
			new CaseInvalid("(int)", (parser)->{parser.type();}),
			new CaseInvalid("(&& int)", (parser)->{parser.type();}),
			new CaseInvalid("(int &&)", (parser)->{parser.type();}),
			//arrayType
			...
			new CaseInvalid("(1).a", (parser)->{parser.type();}),
			new CaseInvalid("(1).a<x,y>", (parser)->{parser.type();}),
			...
			//funcPtrExpr
			new CaseValid("a::b()", (parser)->{parser.expr();}),
			new CaseValid("a::b(a)", (parser)->{parser.expr();}),
			new CaseValid("a::b(a,b,c)", (parser)->{parser.expr();}),
			new CaseValid("a::b(a,b:c,d)", (parser)->{parser.expr();}),
			new CaseValid("a::b<a>()", (parser)->{parser.expr();}),
			new CaseValid("a::b<a,b,c>()", (parser)->{parser.expr();}),
			new CaseValid("a::b<a,b:c,d>()", (parser)->{parser.expr();}),
			//funcCallExpr
			...
			//terneryOpExpr
			new CaseValid("if true: 1 else: 0", (parser)->{parser.expr();}),
			new CaseValid("if true: 1 elseif false: 2 else: 0", (parser)->{parser.expr();}),
			new CaseValid("if true: 1 elseif false: 2 elseif true: 3 else: 0", (parser)->{parser.expr();}),
			new CaseInvalid("if true: 1", (parser)->{parser.expr();}),
			...
			new CaseValid("class Array<int>", (parser)->{parser.expr();}),
			new CaseValid("class ()->()", (parser)->{parser.expr();}),
			//nullConstExpr
			...
			//funcConstExpr
			new CaseValid("()=>0", (parser)->{parser.expr();}),
			new CaseValid("() {return 0;}", (parser)->{parser.expr();}),
			new CaseValid("<T>()=>0", (parser)->{parser.expr();}),
			new CaseValid("<T>() {return 0;}", (parser)->{parser.expr();}),
			new CaseValid("()=>(1,2)", (parser)->{parser.expr();}),
			new CaseValid("(var x)=>0", (parser)->{parser.expr();}),
			new CaseValid("(var x, var y, var z)=>0", (parser)->{parser.expr();}),
			new CaseValid("x=>()", (parser)->{parser.expr();}),
			new CaseValid("(x,y)=>()", (parser)->{parser.expr();}),
			new CaseValid("int x=>()", (parser)->{parser.expr();}),
			new CaseValid("(int x,y)=>()", (parser)->{parser.expr();}),
			...
			new CaseInvalid("\"\"", (parser)->{parser.packageName();}),
			new CaseInvalid("\"abc\"", (parser)->{parser.packageName();}),
