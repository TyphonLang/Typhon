...
public class TestCompiler extends TyphonTest {
    ...
    public static Collection<Object[]> data() {
		return TyphonTest.makeData(new TestCase("void f() {}", (code)->{
			...
			Assert.assertEquals(0, code.tni.errors.size());
		}),new TestCase("void f() {{int:string} m; for var e : m {m[e.key] = e.value;}}", (code)->{
			Assert.assertEquals(0, code.tni.errors.size());
		}),new TestCase("int x = 1; void f() {}", (code)->{
			Assert.assertEquals(0, code.tni.errors.size());
		}),new TestCase("int x = true; void f() {}", (code)->{
			Assert.assertEquals(1, code.tni.errors.size());
		}),new TestCase("class a {int x = 1;} void f() {}", (code)->{
			Assert.assertEquals(0, code.tni.errors.size());
		}),new TestCase("class a {int x = true;} void f() {}", (code)->{
			Assert.assertEquals(1, code.tni.errors.size());
		}),new TestCase("{int a = 3; print(a);} void f() {}", (code)->{
			Assert.assertEquals(0, code.tni.errors.size());
		}),new TestCase("{int a = true; print(a);} void f() {}", (code)->{
			Assert.assertEquals(1, code.tni.errors.size());
		}),new TestCase("class a {int x = 1; {x = x;}} void f() {}", (code)->{
			Assert.assertEquals(0, code.tni.errors.size());
		}),new TestCase("class a {int x = 1; {x = x;}} class b : a {{x = x;}} void f() {}", (code)->{
			Assert.assertEquals(0, code.tni.errors.size());
		...
		}),new TestCase("void f() {throw new Error(\"test\");}", (code)->{
			Assert.assertEquals(0, code.tni.errors.size());
		}),new TestCase("void f() {List<int> a; int b = a[1];}", (code)->{
			Assert.assertEquals(0, code.tni.errors.size());
		}),new TestCase("class A {int x;} class B : A {new(this.x);} void f() {B b = new B(1);}", (code)->{
			Assert.assertEquals(0, code.tni.errors.size());
		}),new TestCase("class A {int x;} class B : A {int getX() => x} void f() {}", (code)->{
			Assert.assertEquals(0, code.tni.errors.size());
		}));
	}
